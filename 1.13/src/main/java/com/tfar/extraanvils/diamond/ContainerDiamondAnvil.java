...
public class ContainerDiamondAnvil extends Container {
  ...
  public ContainerDiamondAnvil(InventoryPlayer playerInventory, final World worldIn, final BlockPos blockPosIn, EntityPlayer player) {
    this.maximumCap = Integer.MAX_VALUE;
    ...
    this.addSlot(new Slot(this.outputSlot, 2, 134, 47) {
      ...
      public boolean canTakeStack(EntityPlayer playerIn) {
        return (playerIn.abilities.isCreativeMode || playerIn.experienceLevel >= ContainerDiamondAnvil.this.maximumCost) && ContainerDiamondAnvil.this.maximumCost > 0 && this.getHasStack();
      }
      ...
      public ItemStack onTake(EntityPlayer playerIn, @Nonnull ItemStack stack) {
        if (!playerIn.abilities.isCreativeMode) {
          playerIn.addExperienceLevel(-ContainerDiamondAnvil.this.maximumCost);
        }
        ...
        if (!playerIn.abilities.isCreativeMode && !worldIn.isRemote && iblockstate.getBlock() instanceof BlockDiamondAnvil && playerIn.getRNG().nextFloat() < breakChance) {
          iblockstate = BlockDiamondAnvil.damage(iblockstate);
      ...
      public float getBreakChance(EntityPlayer playerIn, ItemStack stack){
        return ForgeHooks.onAnvilRepair(playerIn, stack, ContainerDiamondAnvil.this.inputSlots.getStackInSlot(0), ContainerDiamondAnvil.this.inputSlots.getStackInSlot(1))/64;
      }
    });
    ...
    if (itemstack.isEmpty()) {
      ...
      if (!itemstack2.isEmpty()) {
        ...
        if (itemstack1.isDamageable() && itemstack1.getItem().getIsRepairable(itemstack, itemstack2)) {
          int j2 = Math.min(itemstack1.getDamage(), itemstack1.getMaxDamage() / 4);
          ...
          for (k2 = 0; j2 > 0 && k2 < itemstack2.getCount(); ++k2) {
            int l2 = itemstack1.getDamage() - j2;
            itemstack1.setDamage(l2);
            ++i;
            j2 = Math.min(itemstack1.getDamage(), itemstack1.getMaxDamage() / 4);
          }
          ...
          for (Enchantment enchantment1 : map1.keySet()) {
            if (enchantment1 != null) {
              ...
              if (this.thePlayer.abilities.isCreativeMode || itemstack.getItem() == Items.ENCHANTED_BOOK) {
                flag1 = true;
              }
      ...
      if (this.maximumCost >= this.maximumCap && !this.thePlayer.abilities.isCreativeMode) {
        itemstack1 = ItemStack.EMPTY;
      }
  ...
  public boolean canInteractWith (@Nonnull EntityPlayer playerIn){
    return this.theWorld.getBlockState(this.selfPosition).getBlock() instanceof BlockDiamondAnvil && playerIn.getDistanceSq((double) this.selfPosition.getX() + 0.5D, (double) this.selfPosition.getY() + 0.5D, (double) this.selfPosition.getZ() + 0.5D) <= 64.0D;
  }
  ...
  public static boolean onAnvilChange(ContainerDiamondAnvil container, ItemStack left, ItemStack right, IInventory outputSlot, String name, int baseCost) {
